...
class Base():
    ...
    def __init__(self, *args: list, **kwargs: dict):
        ...
        if kwargs.get('created_at') is not None:
            self.registered_at = datetime.strptime(kwargs.get('created_at'),
                                                TIMESTAMP_FORMAT)
        else:
    ...
    def to_json(self, for_serialization: bool = False) -> dict:
        """ Represents the object in a JSON dictionary
        """
        result = {}
    ...
    def load_from_file(cls):
        """ Load all objects representations from the file
        """
    ...
    def save_to_file(cls):
        """ Save all objects representations in the file
        """
        s_class = cls.__name__
        file_path = ".db_{}.json".format(s_class)
        objs_json = {}
    ...
    def save(self):
        """ Save the current object representation
        """
    ...
    def remove(self):
        """ Remove an object representation
        """
        s_class = self.__class__.__name__
    ...
    def get(cls, id: str) -> TypeVar('Base'):
        """ Return one representation by ID
        """
        s_class = cls.__name__
    ...
    def search(cls, attributes: dict = {}) -> List[TypeVar('Base')]:
        """ Search all representations with matching attributes
        """
        s_class = cls.__name__
        def _search(obj):
            if len(attributes) == 0:
